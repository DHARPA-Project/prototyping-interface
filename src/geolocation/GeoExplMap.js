...
const GeoExplMap = (props) => {
    ...
    const [tooltip1, setTooltip1] = useState(props.data[6][0])
    const [tooltip2, setTooltip2] = useState(props.data[6][1])
    const [tooltip3, setTooltip3] = useState(props.data[6][2])
    ...
    let createSingleColorHue = (evt) => {
        ...
        const colorScale = d3.scaleLinear()
            .domain([d3.min(props.reducedData[0], d => +d.count), d3.max(props.reducedData[0], d => +d.count)])
            .range([0, 3])

        const data = props.reducedData[1].map((element, index) => (
            {
                0: element[0], 1: element[1], color: interpolate(colorScale(+props.reducedData[0][index].count)), count: +props.reducedData[0][index].count
            }))

        initParams(width, height, context, container, data, props.data[4], 0, 0)
        attachEvents(evt, width, height, context, container, data, props.data[4], 0)
    ...
    let createClusters = (evt) => {
        console.log(props.reducedData)
        const [width, height, canvas, container, context] = getMapItems()

        const data = props.reducedData[1].map((element, index) => (
            {
                0: element[0], 1: element[1], count: +props.reducedData[0][index].count
            }))

        initParams(width, height, context, container, data, props.reducedData[0], 0, 'grouped')
        attachEvents(evt, width, height, context, container, data, props.reducedData[0], 'grouped')
    ...
    let resetMap = (evt) => {
        ...
        initParams(width, height, context, container, props.reducedData[1], props.data[4], 0, 0)
        attachEvents(evt, width, height, context, container, props.reducedData[1], props.data[4], 0)
        zoomed(evt, width, height, context, props.reducedData[1], 0, 0)
    }
        ...
        switch (value) {
            case 'all':
                data.map((element, index) => {
                    newdata.push({ 0: element[0], 1: element[1], color: findColor(props.reducedData[0][index][selectedCol]) })
                })
                newitemsdata = props.data[4]
                break
            ...
            default:
                ...
                data.map((element, index) => {
                    ...
                    if (props.reducedData[0][index][selectedCol] == val) {
    ...
    let displayCategory = (zoomevt, value, selectedCol) => {
        ...
        let newdata = (value === undefined || value === 'all') ? catData('all', props.reducedData[1], selectedCol) : catData(value, props.reducedData[1], selectedCol)
    ...
    let createCat = (zoomevt, col) => {
        ...
        let uniqueCat = [...new Set(extractColumn(props.data[5], col))]
        ...
        const displayCats = (uniqueCat, col) => {
            ...
            uniqueCat.map((item, index) => {
                const res = props.data[5].filter((item) => {
        ...
        if (mapClickStatus === 'off') {
            ...
            if (status1 == 'mousemove' || status1 == 'mouseclick') {
                ...
                if (status1 === 'mousemove') {
                    context.moveTo(props.reducedData[1][p][0] + getRadius(status2, props.reducedData[0][p]), props.reducedData[1][p][1])
                    context.arc(props.reducedData[1][p][0], props.reducedData[1][p][1], getRadius(status2, props.reducedData[0][p]) * 1.1, 0, 2 * Math.PI)
                }

                if (status1 === 'mouseclick') {
                    context.arc(props.reducedData[1][p][0], props.reducedData[1][p][1], getRadius(status2, props.reducedData[0][p]) * clickCoeff, 0, 2 * Math.PI)
                    mapClickStatus = 'on'
                    context.fill()
                }
    ...
    let mapStats = (points) => {
        const totCircles = points.map((item, index) => +props.reducedData[0][index].count).reduce((prev, next) => prev + next)
        setTotalPoints(props.data[5].length)
    ...
    let findNeighborPoints = (x, y) => {
        const delaunay = Delaunay.from(props.data[4])
    ...
    useEffect(() => {
        ...
        initParams(width, height, context, container, props.reducedData[1], props.data[4], 0, 0)
        attachEvents(0, width, height, context, container, props.reducedData[1], props.data[4], 0)
        zoomed(0, width, height, context, props.reducedData[1], 0, 0)
    ...
    if (tooltipData != null) {
        TooltipContent.push(<Card.Content><span>{tooltip1}: {props.data[5][tooltipData][tooltip1]} | </span><span>{tooltip2}: {props.data[5][tooltipData][tooltip2]} | </span><span>{tooltip3}: {props.data[5][tooltipData][tooltip3]}</span></Card.Content>)
    }
    ...
    return (
        <>
            <Grid>
                    ...
                    <Grid.Column width={7}>
                        <Container >
                            <div style={{ display: tooltipDisplay }}>
                                {tooltipData != null &&
                                    <Card.Content><span>{tooltip1}: {props.data[5][tooltipData][tooltip1]} | </span><span>{tooltip2}: {props.data[5][tooltipData][tooltip2]} | </span><span>{tooltip3}: {props.data[5][tooltipData][tooltip3]}</span></Card.Content>
                                }
                            </div>
                        </Container>
                    </Grid.Column>
                ...
                <Grid.Row>
                    <Grid.Column width={3} style={{ marginLeft: '2%' }}>
                        <GeoMapAccordion colorChange={handleChangeColor} colorStatus={mapColor} colOptions={colOptions} catList={catList} zoomLevel={{ k: kSvg, x: xSvg, y: ySvg }} displayCategory={displayCategory} cols={props.data[6]} tooltipCols={tooltipCols} neighbSizeChange={neighbSizeChange} maxCatReached={maxCatReached} catSel={catSel} />
                    </Grid.Column>
                ...
                <GeoDataFrame neighborsTab={neighborPoints} unmapTab={props.unmapTab} absTab={props.absTab} fullData={props.data[5]} />
